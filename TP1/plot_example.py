...
import matplotlib.pyplot as plt #librería para los gráficos
import numpy as np #librería para vectores
...
t = np.linspace(0,2*np.pi,1000) #haces un arreglo donde entre 0 y 2PI hay 1000puntos
data = 2*np.sin(t) #usas el arreglo en la funcion seno

plt.figure(figsize=(8, 6), dpi=100) #figsize es el tamaño y dpi resolución por puntos en pulgada
plt.subplot(3,2,1) #haces 3 filas y 2 columnas, elegis el primero
plt.plot(t,data) #graficas data respecto al tiempo
...
# Gráfico de barras
plt.bar(tipos_fibra, atenuaciones, color='navy') #creas grafico de barras
...
plt.pie(por/total) #para crear gráfico circular
...
print(caras)
cara = np.random.randint(1,7,int(size_)) #generas size_numeros aleatorios en el intervalo [1,7) 
freq = np.zeros(6)
for ii,c in enumerate(caras): #enumerate crea una colección donde da un indice y toma el valor de cada numero
        aux = np.zeros(int(size_))
        aux[cara == c] = 1 #donde cara=c pone un 1 en las celdas, despues de hacer la mascara booleana de cara==c que devuelve true y false
        freq[ii] = np.sum(aux) #suma la cantidad de veces que salio esa cara
print(freq)
print(cara)
...
plt.hist(cara, [ii for ii in range(1,8)], edgecolor='black',width=0.3,align="mid",density=False) #haces un histograma 
...
plt.xlim(1,6.5) #seteas el rango a representar
plt.ylabel('Freq. de Aparición', fontsize=12)
ax = plt.subplot2grid((3, 2), (2, 0), colspan=2) #permite definir dentro del subplot la ubicacion y además le decis colspan 2 para que ocupe ambas cols
# Datos
distancia = [1, 2, 3, 4, 5]
potencia = [-2.1, -3.5, -5.9, -7.8, -9.7]

# Gráfico de dispersión
plt.scatter(distancia, potencia, color='red', marker='o') #creas un grafico de dispersión
...
# Mostrar cuadrícula
plt.grid(True) #creas la cuadricula
